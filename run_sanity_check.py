                ...
                if pd.notna(current_atr_threshold):
                    if current_row['regime'] == 'low_volatility':
                        if (current_row['close'] < current_row[f'BB_LOWER_{int(short_window)}'] and current_row['close'] > current_row['ma_200'] and current_row['RSI_14'] < rsi_oversold and current_funding_rate < funding_rate_threshold):
                            ...
                            trade_log.append({'date': current_row.name, 'type': 'BUY', 'price': current_row['open'], 'pnl': np.nan})
                            ...
                            position, signal_generated = -1, True
                            trade_log.append({'date': current_row.name, 'type': 'SELL', 'price': current_row['open'], 'pnl': np.nan})
...
if __name__ == '__main__':
    ...
    print(f"Fetching data for sanity check ({start_date} to {end_date})...")
    sanity_check_data = fetch_historical_data(start_date_str=start_date, end_date_str=end_date)
    print("Fetching funding rate data...")
    funding_rates = fetch_funding_rates(since=sanity_check_data.index[0], timeframe='5m')
    sanity_check_data = pd.merge_asof(
        left=sanity_check_data.sort_index(),
    ...
    print("\nRunning sanity check backtest with optimized parameters...")
    ...
    print("\n" + "="*50)
    print("           SANITY CHECK BACKTEST RESULTS")
    print("="*50)
    print(f"Period Analyzed:     {sanity_check_data.index.min()} to {sanity_check_data.index.max()}")
    if not trades.empty:
