    ...
    for i in range(250, len(df)):
        ...
        if position == 0:
            ...
            if pd.notna(current_funding_rate):
                ...
                current_atr_threshold = atr_threshold.get(current_row.name) # Correctly get threshold by date
                if pd.notna(current_atr_threshold):
                    if current_row['regime'] == 'low_volatility':
                        if (current_row['close'] < current_row[f'BB_LOWER_{short_window}'] and current_row['close'] > current_row['ma_200'] and current_row['RSI_14'] < rsi_oversold and current_funding_rate < funding_rate_threshold):
                            position, signal_generated = 1, True
                        elif (current_row['close'] > current_row[f'BB_UPPER_{short_window}'] and current_row['close'] < current_row['ma_200'] and current_row['RSI_14'] > rsi_overbought and current_funding_rate > -funding_rate_threshold):
                            position, signal_generated = -1, True
                    elif current_row['regime'] == 'high_volatility':
                        if (prev_row[f'EMA_{short_window}'] < prev_row[f'EMA_{long_window}'] and current_row[f'EMA_{short_window}'] > current_row[f'EMA_{long_window}'] and current_row['close'] > current_row['ma_200'] and current_row['ATR_14'] > current_atr_threshold and current_funding_rate < funding_rate_threshold):
                            position, signal_generated = 1, True
                        elif (prev_row[f'EMA_{short_window}'] > prev_row[f'EMA_{long_window}'] and current_row[f'EMA_{short_window}'] < current_row[f'EMA_{long_window}'] and current_row['close'] < current_row['ma_200'] and current_row['ATR_14'] > current_atr_threshold and current_funding_rate > -funding_rate_threshold):
                            position, signal_generated = -1, True
...
if __name__ == '__main__':
    start_date, end_date = '2025-02-20', '2025-08-20'
