...
# The correct import for the official HMM distribution
from pymc_experimental.distributions import HiddenMarkovModel
...
OUTPUT_DIR = "hmm_professional_results"
DRAWS = 2000
TUNE = 2000
CHAINS = 4
...
def get_regimes_from_idata(idata):
    """Extracts the most likely regime sequence from the inference data's latent states."""
...
if __name__ == "__main__":
    ...
    with pm.Model() as hmm_model:
        ...
        p_transition = pm.Dirichlet("p_transition", a=np.ones(N_STATES), shape=N_STATES)
        p_initial = pm.Dirichlet("p_initial", a=np.ones(N_STATES))

        # Priors for regime volatilities (std of volatility)
        ...
        mus = pm.HalfNormal(
            ...
            initval=np.array([0.002, 0.008])
        )
        
        # Emission distributions for each state
        emission_dists = pm.Normal.dist(mu=mus, sigma=sigma, shape=(len(train_vol), N_STATES))
        
        # Use the official, robust HiddenMarkovModel distribution
        # It automatically handles the complex likelihood calculation and infers the latent states
        likelihood, states = HiddenMarkovModel(
            "likelihood",
            P=p_transition,
    ...
    idata.to_netcdf(os.path.join(OUTPUT_DIR, "hmm_professional_inference_data.nc"))
    with open(os.path.join(OUTPUT_DIR, "hmm_professional_model.pkl"), "wb") as f: cloudpickle.dump(hmm_model, f)
    
    # Predict regimes on the training set using the inferred latent states
    train_regimes = get_regimes_from_idata(idata)

    # Simplified prediction for the test set
    ...
    model_params = {
        ...
        'p_regime': posterior_data['p_initial'].mean(axis=-1).values
    }
    ...
    ax.set_title(f"Volatility HMM Regimes for {TICKER} (Hourly) - Professional")
    ax.legend()
