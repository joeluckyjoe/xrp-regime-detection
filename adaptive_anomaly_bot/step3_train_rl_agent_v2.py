...
TICKER = "QQQ"
MODEL_SAVE_PATH = "ppo_volatility_agent_v2.pth"
TRAINING_EPISODES = 50
UPDATE_TIMESTEP = 2000

# --- Trading Environment with Reshaped Reward ---
class VolatilityBreakoutEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.position == 0 and is_breakout:
            ...
            if back_to_compression or agent_exits:
                ...
                # Terminal Reward on close
                reward = (self.balance - self.entry_portfolio_value) / self.entry_portfolio_value

                self.shares_held = 0; self.position = 0; self.entry_price = 0
        
        # Continuous reward for holding
    ...
    def _get_portfolio_value(self):
        ...
        if self.position == 1:
            ...
            profit_loss = (self.entry_price - current_price) * self.shares_held
            # The value of a short position is the initial cash minus the current cost to close
...
class PPOAgent:
    def __init__(self, env, lr=3e-4, gamma=0.99, K_epochs=4, eps_clip=0.2):
        self.env=env; self.gamma=gamma; self.eps_clip=eps_clip; self.K_epochs=K_epochs; self.policy=ActorCritic(env.observation_space.shape[0],env.action_space.n); self.optimizer=optim.Adam(self.policy.parameters(),lr=lr); self.policy_old=ActorCritic(env.observation_space.shape[0],env.action_space.n); self.policy_old.load_state_dict(self.policy.state_dict()); self.MseLoss=nn.MSELoss(); self.buffer=RolloutBuffer()
    def select_action(self, state):
        with torch.no_grad():
            state_tensor=torch.FloatTensor(state); action_probs,state_val=self.policy_old(state_tensor); dist=Categorical(action_probs); action=dist.sample()
