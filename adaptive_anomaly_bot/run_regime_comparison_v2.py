...
def generate_vix_regime_labels_quantile(df, bull_quantile=0.4, bear_quantile=0.8):
    ...
    print(f"  VIX Thresholds: BULL < {bull_threshold:.2f}, BEAR > {bear_threshold:.2f}")
...
def run_kmeans_model(df):
    print("Running Method 1: Rolling K-Means (Baseline)...")
    ...
    for i in tqdm(range(window_size, len(df))):
        ...
        bull_idx, bear_idx = np.argmax(centers[:, 1]), np.argmin(centers[:, 1])
...
def run_adaptive_gmm_model_curated(features, df, curated_feature_indices):
    print("Running Method 2: Adaptive GMM (Curated Features)...")
    labels = pd.Series(index=df.index, dtype=str)
    window_size = 252 * 7
    
    curated_features = features[:, curated_feature_indices]
    
    initial_features = curated_features[:window_size]
    gmm = GaussianMixture(n_components=3, random_state=42, n_init=10, covariance_type='full').fit(initial_features)
    ...
    for i in tqdm(range(window_size, len(features))):
        window_features = curated_features[i-window_size:i]
        reg_cov = 1e-6 
        ...
        pred_label = gmm.predict(curated_features[i,:].reshape(1, -1))[0]
...
def plot_regime_comparison(df, filename='regime_comparison_plot.png'):
    ...
    models = ['VIX_GroundTruth', 'KMeans_Labels', 'GMM_Labels']
    titles = ['VIX Ground Truth (Quantile-Based)', 'Method 1: K-Means', 'Method 2: GMM (Curated Features)']
    
    for ax, model, title in zip(axes, models, titles):
        for regime, color in color_map.items():
            for i in np.where(df[model] == regime)[0]:
...
if __name__ == '__main__':
    ...
    print(f"--- Starting v2 Regime Detection Comparison for {TICKER} ---")
    data = fetch_and_prepare_data(ticker=TICKER)
    features, df = calculate_walk_forward_features(data)
    
    # --- Define the 6 curated features for the GMM model ---
    # Structure: 3 timeframes * 9 indicators * 4 values (surprise, raw, mean, scale)
    # Using the raw normalized value for each indicator (index 1 of 4)
    # returns_1h, MACD_1d, vix_1h, atr_1h, ADX_1d, rsi_1d
    curated_indices = [
    ...
    gmm_labels = run_adaptive_gmm_model_curated(features, df, curated_indices)
    ...
    print("\n\n--- REGIME DETECTION COMPARISON REPORT (v2) ---")
    ...
    print("--- Method 1: Rolling K-Means (Baseline) ---")
    ...
    print("--- Method 2: Adaptive GMM (Curated Features) ---")
