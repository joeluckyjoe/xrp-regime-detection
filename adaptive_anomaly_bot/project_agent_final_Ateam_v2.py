...
# --- VERSION CUE ---
SCRIPT_VERSION = "Definitive Agent v6.0 (A-Team Data-Driven Features)"
...
def run_portfolio_walk_forward(tickers, features_dict, dfs_dict, total_training_steps, params_dict, initial_portfolio_balance=100000, save_plot_filename=None, seed=42):
    ...
    # --- FEATURE SETS FOR CLASSIFIER ---
    # Original 6-feature set that works well for GLD and the baseline portfolio
    original_indices = [ (0*36)+(0*4)+1, (1*36)+(5*4)+1, (0*36)+(4*4)+1, (0*36)+(3*4)+1, (1*36)+(8*4)+1, (1*36)+(2*4)+1 ]
    # New 18-feature set, data-driven for USO (and to be used for HYG as well)
    new_indices = [13, 7, 9, 36, 25, 33, 5, 21, 3, 17, 29, 26, 84, 1, 96, 104, 2, 92]
    ...
    for i in range(num_w):
        ...
        for ticker in tickers:
            ...
            # --- ASSET-SPECIFIC FEATURE SELECTION ---
            if ticker in ["USO", "HYG"]:
                curated_indices = new_indices
                print(f"    Using new 18-feature set for {ticker}")
            else:
                curated_indices = original_indices
                print(f"    Using original 6-feature set for {ticker}")
    ...
    if save_plot_filename:
        ...
        ax.set_title(f"A-Team Portfolio Performance (Seed {seed}) - Data-Driven Features")
...
def main():
    ...
    for seed in VERIFICATION_SEEDS:
        ...
        results_df = run_portfolio_walk_forward(
            ...
            save_plot_filename=f'final_runs/equity_curve_seed_{seed}_Ateam_v2.png', seed=seed
        )
    ...
    if final_values:
        ...
        print("\n\n--- FINAL A-TEAM (V2) PORTFOLIO 10-SEED VERIFICATION REPORT ---")
