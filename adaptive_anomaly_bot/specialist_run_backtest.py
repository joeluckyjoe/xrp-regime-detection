...
def create_window_fingerprint(window_df):
    """
    Calculates a statistical fingerprint for a given data window.
    The fingerprint is a vector representing the market's characteristics during that period.
    """
...
def detect_regimes(historical_window_dfs, n_regimes=3, seed=42):
    """
    Takes a list of past data windows, creates fingerprints, and clusters them to define regimes.
    """
...
def map_regimes_to_labels(kmeans, scaler):
    """
    Analyzes the kmeans cluster centers to assign meaningful, stable labels.
    Returns a dictionary mapping the arbitrary cluster index to a label ('BULL', 'BEAR', 'SIDEWAYS').
    """
...
def run_walk_forward_analysis(full_df, all_features, total_training_steps, params, save_plot_filename=None, seed=42):
    ...
    print(f"\n--- Starting Final Backtest ({num_w} windows) ---")
    ...
    if save_plot_filename and not res_df.empty:
        ...
        ax.plot(norm_p, label='v20 Specialist Agent Equity Curve', color='deepskyblue', linewidth=2)
        ...
        current_regime_label = res_df['Regime'].iloc[0]
        for i in range(1, len(res_df)):
            if res_df['Regime'].iloc[i] != current_regime_label:
                ...
                start_date = end_date
                current_regime_label = res_df['Regime'].iloc[i]
        ...
        ax.set_title("v20 'Team of Specialists' Performance", fontsize=16)
...
if __name__ == '__main__':
    ...
    if all_data:
        ...
        v18_params = {'lr': 0.000241, 'entropy_coef': 0.0719}
        params = {'BULL': v18_params, 'BEAR': v18_params, 'SIDEWAYS': v18_params}
        results = run_walk_forward_analysis(df, features, 50000, params, 'equity_curve_final.png', SEED)
