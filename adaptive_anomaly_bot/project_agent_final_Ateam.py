...
# --- VERSION CUE ---
SCRIPT_VERSION = "Definitive Agent v5.2 (A-Team Diagnostic)"
...
def run_portfolio_walk_forward(tickers, features_dict, dfs_dict, total_training_steps, params_dict, VERIFICATION_SEEDS, initial_portfolio_balance=100000, save_plot_filename=None, seed=42):
    ...
    for i in range(num_w):
        ...
        for ticker in tickers:
            ...
            # --- START: DIAGNOSTIC PLOT LOGIC ---
            # Only generate plots for the first seed to avoid clutter
            if seed == VERIFICATION_SEEDS[0] and model is not None:
                print(f"    Generating diagnostic plot for {ticker}, Window {i+1}...")
                # Get the actual predictions for the entire window's data
                predicted_regimes = pd.Series(model.predict(X_window_scaled), index=train_df_window.index)

                fig, ax1 = plt.subplots(figsize=(16, 8))

                # Plot price on the first y-axis
                ax1.plot(train_df_window.index, train_df_window['Close'], label='Price', color='black', zorder=10)
                ax1.set_ylabel('Price')
                ax1.set_xlabel('Date')

                # Create a second y-axis for ATR
                ax2 = ax1.twinx()
                ax2.plot(train_df_window.index, train_df_window['atr'], label='ATR (Volatility)', color='gray', linestyle='--', zorder=10)
                ax2.set_ylabel('ATR (Volatility)')

                # Add colored background for the predicted regime
                y_min, y_max = ax1.get_ylim()
                for regime, color in [('BULL', 'lightgreen'), ('BEAR', 'lightcoral'), ('SIDEWAYS', 'lightyellow')]:
                    ax1.fill_between(predicted_regimes.index, y_min, y_max,
                                     where=(predicted_regimes == regime),
                                     facecolor=color, alpha=0.4, label=f'Predicted {regime}')
                ax1.set_ylim(y_min, y_max) # Ensure ylim is maintained
                ...
                plt.title(f"Diagnostic Plot for {ticker} - Window {i+1} (Seed {seed})")
                plt.savefig(f'diagnostics/diagnostic_{ticker}_window_{i+1}_seed_{seed}.png')
                plt.close(fig)
            # --- END: DIAGNOSTIC PLOT LOGIC ---
    ...
    if save_plot_filename:
        ...
        ax.set_title(f"A-Team Portfolio Performance (Seed {seed})")
...
def main():
    """
    Main function to run the definitive 10-seed verification backtest.
    """
    print(f"--- {SCRIPT_VERSION} ---")
    
    # --- CONFIGURATION ---
    PORTFOLIO = ["GLD", "USO", "HYG"] # <-- A-Team Portfolio
    INITIAL_CAPITAL = 100000
    TOTAL_TRAINING_STEPS_PER_WINDOW = 50000
    VERIFICATION_SEEDS = range(101, 111)

    # The definitive, robustly optimized hyperparameters for the A-Team
    CHAMPION_PARAMS = {
        "GLD": {'BULL':{'lr':0.001000,'entropy_coef':0.0000},'BEAR':{'lr':0.000010,'entropy_coef':0.0000},'SIDEWAYS':{'lr':0.000100,'entropy_coef':0.0000}},
        "USO": {'BULL':{'lr':0.000031,'entropy_coef':0.0762},'BEAR':{'lr':0.000057,'entropy_coef':0.0223},'SIDEWAYS':{'lr':0.000003,'entropy_coef':0.0422}},
        "HYG": {'BULL':{'lr':0.000072,'entropy_coef':0.0111},'BEAR':{'lr':0.000023,'entropy_coef':0.0765},'SIDEWAYS':{'lr':0.000002,'entropy_coef':0.0879}}
    }
    
    # --- EXECUTION ---
    print(f"\n--- Starting 10-Seed Verification for {', '.join(PORTFOLIO)} Portfolio ---")
    os.makedirs('final_runs', exist_ok=True)
    os.makedirs('diagnostics', exist_ok=True) # <-- Create diagnostics directory
    
    all_data = fetch_and_prepare_data(tickers=PORTFOLIO, period="729d", interval="1h")
    if not all_data or len(all_data) != len(PORTFOLIO):
    ...
    for seed in VERIFICATION_SEEDS:
        ...
        results_df = run_portfolio_walk_forward(
            tickers=PORTFOLIO, features_dict=features_dict, dfs_dict=dfs_dict,
            total_training_steps=TOTAL_TRAINING_STEPS_PER_WINDOW, params_dict=CHAMPION_PARAMS,
            VERIFICATION_SEEDS=VERIFICATION_SEEDS,  # <-- Add this line
            initial_portfolio_balance=INITIAL_CAPITAL,
            save_plot_filename=f'final_runs/equity_curve_seed_{seed}_Ateam.png', seed=seed
        )
    ...
    if final_values:
        ...
        print("\n\n--- FINAL A-TEAM PORTFOLIO 10-SEED VERIFICATION REPORT ---")
