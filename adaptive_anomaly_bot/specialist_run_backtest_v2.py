...
# --- New Imports for Supervised Regime Detection ---
from sklearn.linear_model import LogisticRegression
from sklearn.preprocessing import StandardScaler

# --- Imports from Original Project ---
...
# --- Data, Environment, and Agent Code (from specialist_train_rl_agent.py) ---
# NOTE: This is the multi-asset version of the function
...
# --- RL AGENT CLASSES AND TRAINING/TESTING FUNCTIONS ---
# ... (These are the same full classes from specialist_train_rl_agent.py) ...
    ...
    def step(self, action):
        ...
        if len(self.returns_history) > 21:
            ...
            if self.reward_scheme == 'calmar':
                ...
                portfolio_values = np.array(self.portfolio_history); running_max = np.maximum.accumulate(portfolio_values)
...
def train_agent_on_window(train_df, train_features, total_timesteps, params, pretrained_policy=None, seed=42, reward_scheme='sortino'):
    ...
    for timestep in tqdm(range(total_timesteps), desc="    Training", leave=False, ncols=100):
...
def test_agent_on_window(policy_state_dict, test_df, test_features, initial_balance, seed=42):
    ...
    obs_dim = test_env.observation_space.shape[0]; disc_action_dim = test_env.action_space[0].n; cont_action_dim = test_env.action_space[1].shape[0]
...
# --- NEW SUPERVISED REGIME DETECTION ---
...
def run_rolling_supervised_regime_detection(features, df, curated_feature_indices, train_window_size):
    print(f"  Generating regime labels for {df.name} using Rolling Supervised Model...")
    curated_features = features[:, curated_feature_indices]
    labels = pd.Series(index=df.index, dtype=str)
    for i in tqdm(range(train_window_size, len(df)), leave=False, desc=f"    Rolling Classifier ({df.name})"):
        X_window = curated_features[i-train_window_size:i]; df_window = df.iloc[i-train_window_size:i]
        y_window = generate_vix_regime_labels_quantile(df_window)
        scaler = StandardScaler().fit(X_window); X_window_scaled = scaler.transform(X_window)
...
# --- MAIN PORTFOLIO BACKTESTING FUNCTION (UPDATED) ---
def run_portfolio_walk_forward(tickers, features_dict, dfs_dict, total_training_steps, params, initial_portfolio_balance=100000, seed=42):
    train_ws, test_ws = 252*7, 21*7
    
    print("\n--- Pre-computing Regime Timelines for all assets ---")
    regime_labels_dict = {}
    curated_indices = [ (0*36)+(0*4)+1, (1*36)+(5*4)+1, (0*36)+(4*4)+1, (0*36)+(3*4)+1, (1*36)+(8*4)+1, (1*36)+(2*4)+1 ]
    for ticker in tickers:
        df = dfs_dict[ticker]; df.name = ticker
        features = features_dict[ticker]
        regime_labels_dict[ticker] = run_rolling_supervised_regime_detection(features, df, curated_indices, train_ws)
    ...
    print(f"\n--- Starting Final Multi-Asset Portfolio Backtest ({num_w} windows) ---")
    ...
    for i in range(num_w):
        ...
        for ticker in tickers:
            ...
            if train_ei > len(regime_labels_dict[ticker]): continue
            
            window_regime_labels = regime_labels_dict[ticker].iloc[train_si:train_ei]
            mapped_label = window_regime_labels.mode()[0] if not window_regime_labels.empty else 'SIDEWAYS'
    ...
    if not all_portfolio_results: return
    
    final_results_df = pd.concat(all_portfolio_results)
    final_results_df['TotalPortfolioValue'] = final_results_df.sum(axis=1)
    # ... (Plotting can be added here) ...
    print("\n--- Backtest Complete ---")
    print(f"Final Portfolio Value: ${final_results_df['TotalPortfolioValue'].iloc[-1]:,.2f}")
...
if __name__ == '__main__':
    ...
    if all_data and len(all_data) == len(PORTFOLIO_TICKERS):
        ...
        # NOTE: These parameters are placeholders. The next step is to find the optimal
        # parameters for this new architecture using the robust optimizer.
        placeholder_params = {
            'BULL':     {'lr': 0.0001, 'entropy_coef': 0.01},
            'BEAR':     {'lr': 0.00005, 'entropy_coef': 0.05},
            'SIDEWAYS': {'lr': 0.00001, 'entropy_coef': 0.08}
        }
        ...
        run_portfolio_walk_forward(
            ...
            dfs_dict=dfs_dict,
            total_training_steps=5000, # Using a reduced number for faster test runs
