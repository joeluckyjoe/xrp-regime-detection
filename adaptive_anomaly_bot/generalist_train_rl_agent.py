...
# --- Suppress known, non-critical warnings from Scipy ---
warnings.filterwarnings("ignore", category=RuntimeWarning, module='scipy.stats._continuous_distns')


# --- DATA PREPARATION (v18) ---

def fetch_and_prepare_data(ticker="QQQ", period="729d", interval="1h"):
    """
    Fetches 1h data and prepares three resampled, feature-rich DataFrames
    with base, anomaly, and new trend indicators (MACD, ADX).
    """
    print("--- Fetching and Preparing Multi-Scale Data (v18) ---")
    ...
    print("Fetching DAILY data for VIX...")
    ...
    for timeframe, df in dataframes.items():
        print(f"Calculating indicators for {timeframe} data...")
        ...
        # Update required columns to include new indicators
        required_cols = [
            'Open', 'High', 'Low', 'Close', 'Volume', 'returns', 'rsi', 'atr', 'vix',
            'MACD_12_26_9', 'MACDh_12_26_9', 'MACDs_12_26_9', 'ADX_14'
        ]
        processed_dfs[timeframe] = df_processed[required_cols]

    print("--- Multi-Scale Data Preparation Complete ---")
    return processed_dfs


# --- BAYESIAN FEATURE ENGINEERING ---

class BayesianAnomalyDetector:
    # ... (class is unchanged, including jittering logic) ...
    ...
    def _fit_beta(self, data):
        ...
        if data_norm.std() < 1e-6:
            data_norm += np.random.normal(0, 1e-5, len(data_norm))
    ...
    def _fit_gamma(self, data):
        ...
        if data.std() < 1e-6:
            data += np.random.normal(0, 1e-5, len(data))
        try: self.mle_params = gamma.fit(data.clip(0.0001), floc=0)
        except Exception: pass

# --- RL ENVIRONMENT (v15 Architecture) & AGENT ---

# ... (The rest of the file is unchanged as it adapts dynamically) ...
class TradingEnvironment(gym.Env):
    def __init__(self, df, surprise_scores, initial_balance=100000, trade_fee=0.001):
        super().__init__()
        ...
        self.action_space = gym.spaces.Tuple((
            ...
            gym.spaces.Box(low=np.array([1.0, 0.0, 0.0]),
                           high=np.array([3.0, 0.1, 0.2]),
                           dtype=np.float32)
        ))
    ...
    def step(self, action):
        ...
        stop_loss_atr_multiplier, trend_reward_bonus, trade_penalty = continuous_params[0], continuous_params[1], continuous_params[2]
        ...
        if discrete_action == 1:
            if self.position == -1:
                ...
                self.stop_loss_price = current_price - (current_atr * stop_loss_atr_multiplier)
        ...
        if (self.position == 1 and current_price > self.df.iloc[self._current_step]['Open']) or \
           (self.position == -1 and current_price < self.df.iloc[self._current_step]['Open']):
            reward += trend_reward_bonus
...
class ActorCritic(nn.Module):
    def __init__(self, n_observations, n_discrete_actions, n_continuous_actions):
        ...
        self.policy_head = nn.Linear(256, n_discrete_actions)
        self.param_head = nn.Linear(256, n_continuous_actions)
    ...
    def forward(self, x):
        ...
        means=torch.tanh(self.param_head(x))
        stds=torch.exp(self.log_std_head)
        state_value=self.value_head(x)
        return action_probs,means,stds,state_value

class PPOAgent:
    def __init__(self, env, lr=3e-4, entropy_coef=0.01):
        self.env=env
        self.obs_dim=env.observation_space.shape[0]
        self.disc_action_dim=env.action_space[0].n
        self.cont_action_dim=env.action_space[1].shape[0]
        self.cont_action_low=torch.tensor(env.action_space[1].low)
        self.cont_action_high=torch.tensor(env.action_space[1].high)
        self.lr=lr
        self.gamma,self.gae_lambda=0.99,0.95
        self.clip_epsilon,self.entropy_coef,self.value_loss_coef=0.2,entropy_coef,0.5
        self.epochs,self.mini_batch_size=10,64
        self.policy=ActorCritic(self.obs_dim,self.disc_action_dim,self.cont_action_dim)
        self.optimizer=optim.Adam(self.policy.parameters(),lr=self.lr)
        self.buffer=RolloutBuffer()
    def _scale_continuous_action(self,tanh_action):
        return self.cont_action_low+(tanh_action+1.0)*0.5*(self.cont_action_high-self.cont_action_low)
    def select_action(self,state):
        with torch.no_grad():
            state_tensor=torch.FloatTensor(state).unsqueeze(0)
            action_probs,means,stds,value=self.policy(state_tensor)
            cat_dist=Categorical(action_probs);discrete_action=cat_dist.sample()
            norm_dist=Normal(means,stds);continuous_actions_tanh=norm_dist.sample()
            log_prob_discrete=cat_dist.log_prob(discrete_action)
            log_prob_continuous=norm_dist.log_prob(continuous_actions_tanh).sum(dim=-1)
            total_log_prob=log_prob_discrete+log_prob_continuous
            continuous_actions_scaled=self._scale_continuous_action(continuous_actions_tanh)
            action=(discrete_action.item(),continuous_actions_scaled.squeeze(0).numpy())
        return state,action,total_log_prob.item(),value.item()
    def learn(self):
        rewards=torch.tensor(self.buffer.rewards,dtype=torch.float32)
        is_terminals=torch.tensor(self.buffer.is_terminals,dtype=torch.float32)
        ...
        old_discrete_actions=torch.tensor([a[0] for a in self.buffer.actions],dtype=torch.int64)
        old_tanh_actions=torch.tensor(np.array([a[1] for a in self.buffer.actions]),dtype=torch.float32)
        ...
        for _ in range(self.epochs):
            for index in torch.randperm(len(old_states)).split(self.mini_batch_size):
                action_probs,means,stds,state_values=self.policy(old_states[index])
                cat_dist=Categorical(action_probs);norm_dist=Normal(means,stds)
                logprobs=cat_dist.log_prob(old_discrete_actions[index])+norm_dist.log_prob(old_tanh_actions[index]).sum(dim=-1)
                dist_entropy=cat_dist.entropy()+norm_dist.entropy().sum(dim=-1)
                ...
                value_loss=nn.MSELoss()(state_values.squeeze(),returns[index])
