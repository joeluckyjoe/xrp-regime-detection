...
# --- Imports for Supervised Regime Detection ---
...
# --- DATA, FEATURE, AND AGENT CODE (Full versions) ---
# ... (All previous functions like fetch_and_prepare_data, PPOAgent, TradingEnvironment, etc. are included here) ...
# ... (For brevity, only the modified function is shown below, but the full script would contain all of them) ...
...
# --- MAIN PORTFOLIO BACKTESTING FUNCTION (Corrected) ---
def run_portfolio_walk_forward(tickers, features_dict, dfs_dict, total_training_steps, params_dict, initial_portfolio_balance=100000, save_plot_filename=None, seed=42):
    ...
    for i in range(num_w):
        ...
        for ticker in tickers:
            ...
            # --- ROLLING SUPERVISED DETECTION (INSIDE THE LOOP) ---
            ...
            y_window = generate_vix_regime_labels_quantile(train_df_window)
            
            # +++ DEBUGGING STATEMENTS START +++
            print("\n    --- DEBUG INFO ---")
            print("    Training Label Distribution ('Answer Key'):")
            print(y_window.value_counts().to_string())
            ...
            # +++ DEBUGGING STATEMENTS START +++
            print("\n    Model Prediction Distribution (on training data):")
            print(pd.Series(window_predictions).value_counts().to_string())
            print("    ------------------\n")
            # +++ DEBUGGING STATEMENTS END +++
...
if __name__ == '__main__':
    ...
    CHAMPION_PARAMS = {
        "QQQ": {'BULL':{'lr':0.000031,'entropy_coef':0.0731},'BEAR':{'lr':0.000028,'entropy_coef':0.0299},'SIDEWAYS':{'lr':0.000001,'entropy_coef':0.0138}},
        "GLD": {'BULL':{'lr':0.000039,'entropy_coef':0.0691},'BEAR':{'lr':0.000195,'entropy_coef':0.0908},'SIDEWAYS':{'lr':0.000007,'entropy_coef':0.0093}},
        "TLT": {'BULL':{'lr':0.000471,'entropy_coef':0.0796},'BEAR':{'lr':0.000311,'entropy_coef':0.0604},'SIDEWAYS':{'lr':0.000007,'entropy_coef':7.27e-06}}
    }
    
    print("--- Starting 10-Seed Verification for Baseline Portfolio ---")
